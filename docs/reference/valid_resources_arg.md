...
- object:
  ...
  A MizerSim or MizerParams object from which the resources should be
  selected.
...
A vector of resource names, in the same order as specified in the
'resources' argument. If 'return.logical = TRUE' then a logical vector
is returned instead, with length equal to the number of resources, with
